...
class ApplyPatchTool(BaseTool):
    ...
    def execute(self, file_path: str, changes: List[Dict[str, Any]], dry_run: bool = False, agent_safe_mode: bool = False, trace_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            if agent_safe_mode and os.path.exists(file_path) and not dry_run:
                confirm = input(f"SAFE MODE: Confirm applying changes to '{file_path}'? [y/N]: ")
                if confirm.lower() not in ['y', 'yes']:
                    return {"success": False, "error": "Patch application not confirmed by user."}
            ...
            for i, change in enumerate(changes):
                ...
                if not find_text:
                    changes_applied_details.append({
                        "change_index": i, "success": False, "error": "No 'find' text specified"
                    })
                    continue
                ...
                try:
                    if line_number is not None:
                        ...
                        if 0 <= line_idx < len(lines):
                            ...
                            new_line_content = lines[line_idx] # Initialize with current line content

                            if use_regex:
                                new_line_content, num_replacements = re.subn(find_text, replace_text, new_line_content)
                                if num_replacements > 0:
                                    lines[line_idx] = new_line_content
                                    applied_this_change = True
                            else:
                                if find_text in new_line_content:
                                    new_line_content = new_line_content.replace(find_text, replace_text)
                                    lines[line_idx] = new_line_content
                                    applied_this_change = True
                            
                            if applied_this_change:
                                changes_applied_details.append({
                                    "change_index": i, "success": True, "line_number": line_number,
                                    "original_line": original_line, "new_line": lines[line_idx], "operation": operation_type
                                })
                            else:
                                changes_applied_details.append({
                                    "change_index": i, "success": False, "line_number": line_number,
                        ...
                        temp_content = '\n'.join(lines) # Use current state of lines for global search

                        if use_regex:
                            new_content, num_replacements = re.subn(find_text, replace_text, temp_content)
                            if num_replacements > 0:
                                lines = new_content.split('\n') # Update lines array after global regex change
                                changes_applied_details.append({
                                    "change_index": i, "success": True,
                                    "occurrences_replaced": num_replacements, "global_replacement": True, "operation": operation_type
                                })
                                applied_this_change = True
                            else:
                                changes_applied_details.append({
                                    "change_index": i, "success": False,
                            ...
                            if find_text in temp_content:
                                ...
                                new_content = temp_content.replace(find_text, replace_text)
                                lines = new_content.split('\n') # Update lines array after global string change
                                changes_applied_details.append({
                                    "change_index": i, "success": True,
                                    "occurrences_replaced": occurrences, "global_replacement": True, "operation": operation_type
                                })
                                applied_this_change = True
                            else:
                                changes_applied_details.append({
                    ...
                    changes_applied_details.append({
                        "change_index": i, "success": False, "line_number": line_number,
                        "error": f"Invalid regex pattern '{find_text}': {str(e_regex)}", "operation": "regex"
                    })
                except Exception as e_line: # Catch other unexpected errors during change application
                    changes_applied_details.append({
                        "change_index": i, "success": False, "line_number": line_number,
            ...
            successful_individual_changes = sum(1 for chg_detail in changes_applied_details if chg_detail.get('success'))
